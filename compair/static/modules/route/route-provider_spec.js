describe('user-module', function () {
    ...
    describe('Routes:', function () {
        ...
        describe('"/course/:courseId/assignment/:assignmentId/answer/create"', function() {
            var path = '/course/'+mockCourseId+'/assignment/'+mockAssignmentId+'/answer/create';

            it('should handle pre-loading errors', function() {
                $httpBackend.expectGET('/api/courses/'+mockCourseId).respond(404, '');
                $httpBackend.expectGET('/api/courses/'+mockCourseId+'/assignments/'+mockAssignmentId).respond({});
                $httpBackend.expectGET('/api/courses/'+mockCourseId+'/assignments/'+mockAssignmentId+'/answers/user?draft=true&unsaved=true').respond({});
                $httpBackend.expectGET('/api/timer').respond({});
                $httpBackend.expectGET('modules/answer/answer-form-partial.html').respond('');
                ...
                expect(toaster.error).toHaveBeenCalled();
                expect($rootScope.routeResolveLoadError).not.toBeUndefined();
                expect($route.current.templateUrl).toBe('modules/answer/answer-form-partial.html');
                expect($route.current.controller).toBe('AnswerWriteController');
            });
        ...
        describe('"/course/:courseId/assignment/:assignmentId/answer/:answerId/edit"', function() {
            var path = '/course/'+mockCourseId+'/assignment/'+mockAssignmentId+'/answer/'+mockAnswerId+'/edit';

            it('should handle pre-loading errors', function() {
                $httpBackend.expectGET('/api/courses/'+mockCourseId).respond(404, '');
                $httpBackend.expectGET('/api/courses/'+mockCourseId+'/assignments/'+mockAssignmentId).respond({});
                $httpBackend.expectGET('/api/courses/'+mockCourseId+'/assignments/'+mockAssignmentId+'/answers/'+mockAnswerId).respond({});
                $httpBackend.expectGET('/api/timer').respond({});
                $httpBackend.expectGET('modules/answer/answer-form-partial.html').respond('');
