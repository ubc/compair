...
def create_course():
    ...
    usertype = User.query.filter_by(id = user.id).first().userrole.id
    table = Enrollment(user.id, course.id, usertype)
    db_session.add(table)
...
def list_course():
    ...
    for course in courses:
        ...
        lst.append( {"id": course.id, "name": course.name, "role": role, "count": len(course.question), "new": new, "time": time} )
    db_session.rollback()
