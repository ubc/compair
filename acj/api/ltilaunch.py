    ...
    def get(self):
        ...
        "status": {
            ...
            "user": {
                ...
                "displayname": user display name; autogenerated
                "email": user's email
            }
        }
        """
        ...
        body = {
            'valid' : True if sess.get('LTI') else False
        }

        assignment = {
            'id': lti_resource_link.acj_assignment_id if lti_resource_link else None,
            'exists': True if lti_resource_link.acj_assignment_id else False
        }

        course = {
            'name': lti_context.context_title if lti_context else None,
            'id': lti_context.acj_course_id if lti_context.acj_course_id else None,
            'exists': True if lti_context.acj_course_id else False,
            'course_role': lti_user_resource_link.course_role.value if lti_user_resource_link else None
        }

        lti_user = LTIUser.query.get(sess['lti_user'])
        user = {
            'exists': True if lti_user.acj_user_id else False,
