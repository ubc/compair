...
class AnswerCommentsAPITests(ACJTestCase):
    ...
    def _build_url(self, course_id, question_id, answer_id, comment_id = None):
        url = '/api/courses/' + str(course_id) + '/questions/' + str(question_id) + \
    ...
    def test_get_all_answer_comments(self):
        ...
        self.assertEqual(self.data.get_answer_comments_by_question(self.questions[0])[1].content,
    ...
    def test_get_single_answer_comment(self):
        ...
        url = self._build_url(self.course.id, self.questions[0].id,
        ...
        # test invalid course id
        self.login(self.data.get_authorized_instructor().username)
        invalid_url = self._build_url(999, self.questions[0].id,
                    self.answers[self.questions[0].id][0].id, comment.id)
        rv = self.client.get(invalid_url)
        self.assert404(rv)
        ...
        invalid_url = self._build_url(self.course.id, self.questions[0].id,
    ...
    def test_get_user_answer_comments(self):
        url = '/api/courses/' + str(self.course.id) + '/questions/' + str(self.questions[0].id) + \
        ...
        # test invalid course id
        self.login(self.data.get_extra_student1().username)
        invalid_url = '/api/courses/999/questions/' + str(self.questions[0].id) + \
            '/answers/' + str(self.answers[self.questions[0].id][0].id) + '/users/comments'
        rv = self.client.get(invalid_url)
        self.assert404(rv)

        # test invalid question id
        invalid_url = '/api/courses/' + str(self.course.id) + '/questions/999' + \
            '/answers/' + str(self.answers[self.questions[0].id][0].id) + '/users/comments'
        rv = self.client.get(invalid_url)
        self.assert404(rv)

        # test invalid answer id
        invalid_url = '/api/courses/' + str(self.course.id) + '/questions/' + str(self.questions[0].id) + \
